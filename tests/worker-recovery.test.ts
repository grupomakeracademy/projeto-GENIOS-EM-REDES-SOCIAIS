...
vi.mock('@/lib/supabase/server', () => ({adminClient: () => ({
  rpc: async () => ({data:[state.job],error:null}),
  from: (table:string) => {
...
beforeEach(() => {
  state.writes=[];state.schedules=[];state.job.type='agent_run';state.job.attempts=1;
...
it('enqueues due routines with an idempotency key and advances their schedule', async () => {
  ...
  await tick();
  expect(state.writes).toContainEqual({table:'background_jobs',value:expect.objectContaining({idempotency_key:'agent:2026-09-14T11:00:00Z',payload:{agent_id:'agent',origin:'routine'}})});
